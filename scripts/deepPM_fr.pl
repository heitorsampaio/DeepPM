...
$outdir_stage1 = "$out_dir/DeepPM_stage1";
if(!(-d $outdir_stage1))
{
...
#goto FOLD_RECOG;
#goto SUMMARY;
#system("/home/casp13/DeepPM_3d/scripts/P0_prepare_features/gen_query_files.pl $option_file $query_file $query_dir");
print("$DeepPM_dir/P0_prepare_features/gen_query_files.pl $option_file $query_file $query_dir\n\n");
system("$DeepPM_dir/P0_prepare_features/gen_query_files.pl $option_file $query_file $query_dir");
print "done.\n";
...
print("$DeepPM_dir/P0_prepare_features/hhsearch_align_prepare.pl $prosys_dir $new_hhsearch_dir $psipred_dir $qname.fas $qname.shhm");
system("$DeepPM_dir/P0_prepare_features/hhsearch_align_prepare.pl $prosys_dir $new_hhsearch_dir $psipred_dir $qname.fas $qname.shhm");
chdir $cur_dir;
###########################################################################
...
print "DeepPM job id is $id-$datetarget\n";
...
{
	print("perl  $DeepPM_dir/P1_run_fold_recognition/run_DeepPM_fr.pl  $query_file_new\n");
	system("perl  $DeepPM_dir/P1_run_fold_recognition/run_DeepPM_fr.pl  $query_file_new");
}
...
{
	print "DeepPM fold recognition is finished\n\n";
	exit;
}
...
print("$DeepPM_dir/P2_alignment_generation/gen_query_temp_align_proc.pl $option_file.add $query_file $fr_template_lib_file $query_dir/$qname.all.sel $align_file\n\n");
system("$DeepPM_dir/P2_alignment_generation/gen_query_temp_align_proc.pl $option_file.add $query_file $fr_template_lib_file $query_dir/$qname.all.sel $align_file");
print "done.\n";
...
print("$DeepPM_dir/P2_alignment_generation/gen_query_temp_align_proc.pl $option_file.add $query_file $fr_template_lib_file $query_dir/$qname.hhm.sel $align_file\n");
system("$DeepPM_dir/P2_alignment_generation/gen_query_temp_align_proc.pl $option_file.add $query_file $fr_template_lib_file $query_dir/$qname.hhm.sel $align_file");
print "done.\n";
...
print("$DeepPM_dir/P2_alignment_generation/gen_query_temp_align_proc.pl $option_file.add $query_file $fr_template_lib_file $query_dir/$qname.complete.sel $align_file\n");
system("$DeepPM_dir/P2_alignment_generation/gen_query_temp_align_proc.pl $option_file.add $query_file $fr_template_lib_file $query_dir/$qname.complete.sel $align_file");
print "done.\n";
...
foreach $type (@suffix)
{
	...
	{
		print("$DeepPM_dir/P2_alignment_generation/combine_fr_global_DeepPM.pl $prosys_dir/ . $query_dir/$qname.complete.sel $type 5 $type-comb.pir\n");
		system("$DeepPM_dir/P2_alignment_generation/combine_fr_global_DeepPM.pl $prosys_dir/ . $query_dir/$qname.complete.sel $type 5 $type-comb.pir");
	}elsif($type eq 'hhs')
	{
		print("$DeepPM_dir/P2_alignment_generation/combine_fr_global_DeepPM.pl $prosys_dir/ . $query_dir/$qname.hhm.sel $type 5 $type-comb.pir\n");
		system("$DeepPM_dir/P2_alignment_generation/combine_fr_global_DeepPM.pl $prosys_dir/ . $query_dir/$qname.hhm.sel $type 5 $type-comb.pir");
	}elsif($type eq 'spem')
	{
		print("$DeepPM_dir/P2_alignment_generation/combine_fr_global_DeepPM.pl $prosys_dir/ . $query_dir/$qname.all.sel $type 5 $type-comb.pir\n");
		system("$DeepPM_dir/P2_alignment_generation/combine_fr_global_DeepPM.pl $prosys_dir/ . $query_dir/$qname.all.sel $type 5 $type-comb.pir");
	}
	for ($i = 1; $i <= $top_num; $i++)
	{
		$file = "$type$i.pir";
...
print("$DeepPM_dir/P3_model_generation/gen_model_proc.pl  $prosys_dir $modeller_dir $atom_folder $query_dir $list_file $num_model_simulate $thread_num $qname\n");
system("$DeepPM_dir/P3_model_generation/gen_model_proc.pl  $prosys_dir $modeller_dir $atom_folder $query_dir $list_file $num_model_simulate $thread_num $qname");
...
EVA:
########### model evaluation
$DeepPM_alndir = "$query_dir/alignments/";
$DeepPM_modeldir = "$query_dir/models/";
$DeepPM_evadir = "$query_dir/eva/";
if(!(-d $DeepPM_alndir))
{
	system("mkdir -p $DeepPM_alndir");
}
if(!(-d $DeepPM_modeldir))
{
	system("mkdir -p $DeepPM_modeldir");
}

if(!(-d $DeepPM_evadir))
{
	system("mkdir -p $DeepPM_evadir");
}
system("mv *pdb $DeepPM_modeldir");
system("mv *pir $DeepPM_alndir");

$SBROD_starttime = time();
chdir("$GLOBAL_PATH/software/SBROD");
$cmd = "./assess_protein $DeepPM_modeldir/*pdb &> $DeepPM_evadir/SBROD_ranking.txt";
...
print "Checking $DeepPM_evadir/SBROD_ranking.txt\n";
open(TMPF,"$DeepPM_evadir/SBROD_ranking.txt") || die "Failed to open file $DeepPM_evadir/SBROD_ranking.txt\n";
open(TMPO,">$DeepPM_evadir/Final_ranking.txt") || die "Failed to open file $DeepPM_evadir/Final_ranking.txt\n";
%mod2score=();
while(<TMPF>)
{
	$li = $_;
...
$rankf = "$DeepPM_evadir/Final_ranking.txt";
print "\n\n###################### Start to format the top 5 models, rename the target id to CAMEO id\n";

$DeepPM_top5dir = "$query_dir/TOP5/";
$DeepPM_top5dir_aln = "$query_dir/TOP5_aln/";
$DeepPM_top5dir_temp = "$query_dir/TOP5/temp/";
if(!(-d $DeepPM_top5dir))
{
	system("mkdir -p $DeepPM_top5dir");
}
if(!(-d $DeepPM_top5dir_temp))
{
	system("mkdir -p $DeepPM_top5dir_temp");
}

if(!(-d $DeepPM_top5dir_aln))
{
	system("mkdir -p $DeepPM_top5dir_aln");
}
...
foreach (@rankmodel)
{
	...
	$mod_pir = "$DeepPM_alndir/$mod_prefix.pir";
	if(!(-e $mod_pir))
	{
	...
	$modid++;
	print("### cp $modelfile $DeepPM_top5dir/casp$modid.pdb\n");
	system("cp $modelfile $DeepPM_top5dir/casp$modid.pdb");
	print("### cp $mod_pir $DeepPM_top5dir_aln/casp$modid.pir\n");
	system("cp $mod_pir $DeepPM_top5dir_aln/casp$modid.pir");
}
...
for($i=1;$i<=5;$i++)
{
	...
	if(-e $file)
	{
		`cp $Top5folder/casp$i.pdb $Top5folder/DeepPM$i.pdb`;
	}
	$file_aln = "$DeepPM_top5dir_aln/casp$i.pir";
	if(-e $file)
	{
		`cp $DeepPM_top5dir_aln/casp$i.pir $Top5folder/DeepPM$i.pir`;
	}
}
	...
	while(<TMPIN>)
	{
		...
		foreach $type (@suffix)
		{
			...
			if(!(-e $aln_summary_forweb_align))
			{
				...
				if(!(-d "$outdir_stage1/DeepPM-$id-results/$id"))
				{
					`mkdir $outdir_stage1/DeepPM-$id-results/$id`;
				}
				`cp $aln_summary_forweb_align $outdir_stage1/DeepPM-$id-results/$id/`;
			}
			...
			if(!(-e $aln_summary_forweb_msa))
			{
				...
				`cp $aln_summary_forweb_msa $outdir_stage1/DeepPM-$id-results/$id/`;
				`cp $aln_summary_forweb_msa $outdir_stage1/DeepPM-$id-results/$id/SCOP_fold_Top${fold_index}_top10_$method_name.msa`;
			}
	...
	while(<TMPIN>)
	{
		...
		foreach $type (@suffix)
		{
			...
			if(!(-e $aln_summary_forweb_msa))
			{
				...
				`cp $aln_summary_forweb_msa $outdir_stage1/DeepPM-$id-results/$id/`;
				`cp $aln_summary_forweb_msa $outdir_stage1/DeepPM-$id-results/$id/ECOD_X_fold_Top${fold_index}_top10_$method_name.msa`;
			}
	...
	while(<TMPIN>)
	{
		...
		foreach $type (@suffix)
		{
			...
			if(!(-e $aln_summary_forweb_msa))
			{
				...
				`cp $aln_summary_forweb_msa $outdir_stage1/DeepPM-$id-results/$id/`;
				`cp $aln_summary_forweb_msa $outdir_stage1/DeepPM-$id-results/$id/ECOD_H_fold_Top${fold_index}_top10_$method_name.msa`;
			}
		}
	}
	close TMPIN;
...
system("echo \"DeepPM-3D modeling finish!\" > $query_dir/DeepPM-3D.done "); 
$finish_time = time();
$DeepPM_diff_hrs = ($finish_time -$start_time)/3600;
print "DeepPM-3D modeling finish! Total time: <$DeepPM_diff_hrs> hrs!\n";
...
print "The ranking of top selected templates are saved in $query_dir/$qname.template.rank\n\n";
print "The predicted models are saved in $DeepPM_top5dir\n\n";
