...
while(<IN1>){
	...
	foreach $line (@content)
	{
		...
		`cp $pdbfile $outputdir/DeepPM_top${rank}_model.pdb`;
		...
		print OUT "$line\t$tempname\t$KL\t$temlabel\t$pdbfile\t$outputdir/DeepPM_top${rank}_model.pdb\t$des\t$temlabel_ecodid\t$fa_des\t$temlabel_ecodidfamily\n";
	}
	close OUT;
}
